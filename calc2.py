...
class Calculator(tk.Frame):
    ...
    def __init__(self, master):
        ...
        button_0.grid(row=4, column=0, pady=3, padx=(5, 2), sticky="ew")
        button_point = ttk.Button(
            master, text='.', command=lambda: self.show('.'), bootstyle="info, outline", padding=(0, 15))
        button_point.grid(row=4, column=1, padx=2, pady=3, sticky="ew")
        button_add = ttk.Button(
            master, text='+', command=lambda: self.show('+'), bootstyle="info, outline", padding=(0, 15))
        button_add.grid(row=4, column=2, pady=3, padx=2, sticky="ew")

        button_sub = ttk.Button(
            master, text='-', command=lambda: self.show('-'), bootstyle="secondary, outline", padding=(0, 15))
        button_sub.grid(row=5, column=0, pady=3, padx=(5, 2), sticky="ew")
        button_div = ttk.Button(
            master, text='÷', command=lambda: self.show('/'), bootstyle="secondary, outline", padding=(0, 15))
        button_div.grid(row=5, column=1, pady=3, padx=2, sticky="ew")
        button_mul = ttk.Button(
            master, text='×', command=lambda: self.show('*'), bootstyle="secondary, outline", padding=(0, 15))
        button_mul.grid(row=5, column=2, pady=3, padx=2, sticky="ew")
        button_clear = ttk.Button(
            master, text='c', command=self.clear, bootstyle="success, outline", padding=(0, 15))
        button_clear.grid(row=6, column=0, pady=3, padx=(5, 2), sticky="ew")
        button_eq = ttk.Button(
            master, text='=', command=self.solve, bootstyle="primary", padding=(0, 15))
        button_eq.grid(row=6, column=1, pady=3, padx=2,
